...
class UploadService {
  ...
  async uploadAvatar(
    ...
    formData.append('folder', 'wandermind/avatars'); // Organize uploads in folders
    formData.append('transformation', 'c_fill,g_face,h_400,w_400'); // Auto-crop to 400x400
   ...
   */
  async uploadDestinationImage(file: File): Promise<string> {
    if (!this.cloudinaryCloudName || !this.uploadPreset) {
    ...
    formData.append('file', file);
    formData.append('upload_preset', this.uploadPreset);
    formData.append('folder', 'wandermind/destinations');
    formData.append('transformation', 'c_fill,h_600,w_800'); // 800x600 destination images
    ...
    try {
      ...
      throw new Error('Failed to upload destination image');
    }
  }
